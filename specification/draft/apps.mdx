...
app.oncalltool = async (params, extra) => {
  if (params.name === "tictactoe_move") {
    ...
    const newBoard = makeMove(
      params.arguments.position,
      params.arguments.player,
    );
...
app.registerTool(
  ...
  {
    description:
      "Get current game state including board, current player, and winner",
    outputSchema: z.object({
      ...
      content: [
        {
          type: "text",
          text:
            `Player ${board[position]} moved to position ${position}` +
            (winner ? `. ${winner} wins!` : ""),
        },
      ],
      structuredContent: {
...
Clients and servers negotiate MCP Apps support through the standard MCP
extensions capability mechanism (defined in SEP-1724). Features of MCP Apps
are settings of the existing `io.modelcontextprotocol/ui` extension. They do
not introduce additional extension identifiers.
...
- `mimeTypes`: Array of supported content types (REQUIRED, e.g.,
  `["text/html;profile=mcp-app"]`)
...
### App (Guest UI) Capability Negotiation
